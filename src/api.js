...
const getToken = async (code) => {
  ...
  const encodeCode = encodeURIComponent(code);
  const { access_token } = await fetch(
    `https://0tq0fabzz1.execute-api.us-east-1.amazonaws.com/dev/api/token/${encodeCode}`
  )
    .then((res) => {
      return res.json();
    })
...
export const getEvents = async () => {
  ...
  if (token) {
    ...
    const url = `https://0tq0fabzz1.execute-api.us-east-1.amazonaws.com/dev/api/get-calendar-events/${token}`;
    const result = await axios.get(url);
    if (result.data) {
      var locations = extractLocations(result.data.events);
      localStorage.setItem('lastEvents', JSON.stringify(result.data));
...
export const getAccessToken = async () => {
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      ...
      const results = await axios.get(
        "https://0tq0fabzz1.execute-api.us-east-1.amazonaws.com/dev/api/get-auth-url"
      );
      const { authUrl } = results.data;
      return (window.location.href = authUrl);
    }
